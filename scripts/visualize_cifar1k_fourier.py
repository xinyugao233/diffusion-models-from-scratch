...
def load_first_1000(root: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    images = np.asarray(payload[b"data"][:1000], dtype=np.uint8).reshape(-1, 3, 32, 32)
...
def normalize_rgb(
    values: np.ndarray, low: float = 1.0, high: float = 99.0
) -> Image.Image:
...
def save_subset_contact(
    images: np.ndarray, labels: np.ndarray, path: Path
) -> list[int]:
    ...
    draw.text(
        (12, 10),
        "100 evenly spaced examples from the exact first 1,000 CIFAR-10 training images",
        fill="black",
    )
    for position, index in enumerate(indices):
        ...
        draw_centered(
            draw, (x + cell // 2, y + cell), f"{index}: {LABELS[labels[index]]}"
        )
    canvas.save(path)
    return indices


def save_fourier_overview(
    images: np.ndarray, labels: np.ndarray, path: Path
) -> list[int]:
    ...
    canvas = Image.new(
        "RGB", (left + len(columns) * cell, header + len(indices) * row_h), "white"
    )
    draw = ImageDraw.Draw(canvas)
    ...
    for row, index in enumerate(indices):
        ...
        reconstructed = np.fft.ifft2(
            np.fft.ifftshift(spectrum, axes=(-2, -1)), norm="ortho"
        ).real
        reconstructed_u8 = (
            np.clip((reconstructed + 1.0) * 127.5, 0, 255).round().astype(np.uint8)
        )
        y = header + row * row_h
        ...
        tiles = (
            ...
            Image.fromarray(
                np.moveaxis((phase * 255).astype(np.uint8), 0, -1), "RGB"
            ).resize((128, 128)),
        ...
        draw.text(
            (left + 3 * cell + 4, y + 130), f"max error {error:.1e}", fill="black"
        )
    canvas.save(path)
    return indices
...
def save_six_channels(images: np.ndarray, labels: np.ndarray, path: Path) -> list[int]:
    ...
    canvas = Image.new(
        "RGB", (left + len(titles) * cell, header + len(indices) * row_h), "white"
    )
    draw = ImageDraw.Draw(canvas)
    ...
    for row, index in enumerate(indices):
        ...
        draw.text(
            (left + cell, y + 98),
            "gray=0, white=+, black=- (per-panel robust scale)",
            fill="black",
        )
    canvas.save(path)
    return indices
...
def save_histogram(labels: np.ndarray, path: Path) -> None:
    ...
    for index, count in enumerate(counts):
        ...
        draw.rectangle(
            (x, top + plot_h - bar_h, x + bar_w, top + plot_h), fill="#2563eb"
        )
        draw_centered(
            draw, (x + bar_w // 2, top + plot_h - bar_h - 18), str(int(count))
        )
        draw_centered(draw, (x + bar_w // 2, top + plot_h + 10), LABELS[index])
    draw.text(
        (left, 15),
        "Class counts in the first 1,000 canonical CIFAR-10 training examples",
        fill="black",
    )
    canvas.save(path)
...
def main() -> int:
    ...
    contact_indices = save_subset_contact(
        images, labels, output_dir / "cifar1k_examples_100.png"
    )
    overview_indices = save_fourier_overview(
        images, labels, output_dir / "fourier_examples_overview.png"
    )
    channel_indices = save_six_channels(
        images, labels, output_dir / "fourier_six_channels.png"
    )
    ...
    manifest = {
        ...
        "class_counts": {
            LABELS[i]: int(v) for i, v in enumerate(np.bincount(labels, minlength=10))
        },
    ...
    (output_dir / "subset_visual_manifest.json").write_text(
